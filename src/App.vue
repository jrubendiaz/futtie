...
let canClick = false
let canAccept = false
let canDismissNotification = false
const navigateBackButton = (): Element => document.querySelector(ELEMENT_SELECTOR.BACK_BUTTON) as Element
const searchButton = (): Element => document.querySelector(ELEMENT_SELECTOR.SEARCH_BUTTON) as Element

var intervalAtLeastOneItem: number;
var intervalNoResults: number;
...
const setClock = (miliseconds: number) => {
...
const dispatchClick = (node: Element) => {
...
const triggerMouseEvent = async (node: Element, eventType: string) => {
    try {
...
var intervalAceptar: number | null = null
var intervalItemClick: number | null = null
var intervalErrorNotification: number | null = null
var imageIntervals: number[] = []
const imageReady = (): Promise<{item: Element}> => {
    return new Promise(resolve => {
        imageIntervals = [...imageIntervals, setInterval(() => {
            if(document.querySelector('.tns-slide-active .large.player.item.ut-item-loaded')) {
                resolve({
                    item: document.querySelector('.tns-slide-active .large.player.item.ut-item-loaded') as Element
                })
            }
            if(document.querySelectorAll('.large.player.item.ut-item-loaded').length === 1) {
                resolve({
                  item: document.querySelector('.large.player.item.ut-item-loaded') as Element
                })
            }
        }, 100)]
    })
...
const buyIt = (): Promise<{response: string}> => {
    canClick = true
    canAccept = false
...
var getResults = (): Promise<{response: string}> => {
    ...
    return new Promise(resolve => {
        intervalAtLeastOneItem = setInterval(() => {
            ...
            if(results && results.length > 0 && !resultsAlready) {
                ...
                const clickableItem = lastItem.querySelector('.rowContent') as Element
                try {
...
var search = async() => {
    const minusPlusBtns = (index: number) => document.querySelectorAll('.btn-standard')[index]
    ...
    const inputs = (index: number): HTMLInputElement => document.querySelectorAll('.ut-number-input-control')[index] as HTMLInputElement
    const minBidInput = () => inputs(0)
    const minBuyInput = () => inputs(2)
    const maxBuyInput = () => inputs(3)

    const searchButton = (): Element => document.querySelector('.btn-standard.call-to-action') as Element
    pr.innerHTML = '';
    intervals.forEach(intervalID => {
        clearInterval(intervalID)
    })
    intervals = []
    const getRandomSleep = (max: number, min: number) => {
    ...
    if(parseInt(minBidInput()?.value) > MAX_MIN_BID) {
        decrease = true
    } 
    ...
    if(response === 'CLICKED_RESULT') {
        ...
        if(buyResponse.response === 'BUY_OK') {
            ...
            const buttonGroup : Element = document.querySelector(ELEMENT_SELECTOR.PLAYER_DETAIL.BUTTON_GROUP) as Element
            const playerButtons = buttonGroup.querySelectorAll('button')
            if (buttonGroup && playerButtons) {
              const sendToPileButton = playerButtons[7]
              dispatchClick(sendToPileButton)
              await sleep(200)
...
const minusPlusBtns = (index: number): Element => document.querySelectorAll('.btn-standard')[index]
const minBidMinus = () => minusPlusBtns(0)
const minBidPlus = () => minusPlusBtns(1)
const buyNowMinMinus = () => minusPlusBtns(4)
const buyNowMinPlus = () => minusPlusBtns(5)
